...
def in_profit(fallback_price=None):
    p=get_price()
    if p is None:
        p=fallback_price
        if p is not None: log("Usando precio fallback:"+str(p))
...
def exec_tp(label,pct,flag,fallback_price=None):
    with state_lock:
        if state.get(flag,False): log(label+" ya ejecutado"); return
        if not in_profit(fallback_price): log(label+" en perdida"); return
        cs,side=pos_size()
        if cs==0: return
        ts=int(round(cs*pct))
        if ts<1: return
