...
def process(data):
    action=data.get("action"); log("Procesando:"+str(action))
    if action=="long":
        with state_lock: al=state["position"]=="long"; hp=state.get("last_tp_profit",0)>0
        if al:
            if hp: add_margin()
        ...
        with state_lock: state["position"]="long"; state["entry_price"]=float(str(data.get("price"))); reset(); save_state(state)
    elif action=="short":
        with state_lock: ash=state["position"]=="short"; hp=state.get("last_tp_profit",0)>0
        if ash:
            if hp: add_margin()
        ...
        with state_lock: state["position"]="short"; state["entry_price"]=float(str(data.get("price"))); reset(); save_state(state)
    elif action=="tp1":
...
if __name__=="__main__":
    print("MWhalekiller FART 15m v1 | TP1 50% RSI15m 58/40 | TP2 25% RSI15m 65/33 | Resto 25% libre")
